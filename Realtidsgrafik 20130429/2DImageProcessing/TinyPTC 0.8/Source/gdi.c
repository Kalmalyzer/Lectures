...
void (*ptc_keydowncallback)(WPARAM wParam) = 0;
...
static LRESULT CALLBACK WndProc(HWND hWnd,UINT message,WPARAM wParam,LPARAM lParam)
{
	...
	switch (message)
	{
        ...
        case WM_KEYDOWN:
        {
			...
			if (ptc_keydowncallback)
				ptc_keydowncallback(wParam);
...
int ptc_open(wchar_t *title,int width,int height)
{
	RECT rect;
	int cc;
...

    {
        ...
        AppendMenu(menu, MF_STRING, SC_ZOOM_1x1, L"Zoom 1 x 1");
        AppendMenu(menu, MF_STRING, SC_ZOOM_2x2, L"Zoom 2 x 2");
        AppendMenu(menu, MF_STRING, SC_ZOOM_4x4, L"Zoom 4 x 4");
    }
